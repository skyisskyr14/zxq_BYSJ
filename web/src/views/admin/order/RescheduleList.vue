<template>
  <div>
    <page-header title="改期审核" desc="处理改期申请" />
    ...
    <data-table :data="paged" :loading="loading" class="mt-16">
      ...
      <el-table-column prop="newRange" label="新日期">
        <template slot-scope="scope">{{ scope.row.newRange.join(' 至 ') }}</template>
      </el-table-column>
    ...
    <el-dialog title="改期审核" :visible.sync="dialogVisible" width="420px">
...
import { list, adminAuditReschedule } from '@/mock'
...
export default {
  ...
  computed: {
    filtered() {
      return this.reschedules.filter(r => {
  ...
  methods: {
    ...
    submit() {
      this.$refs.form.validate(valid => {
        if (!valid) return
        adminAuditReschedule(this.form.id, this.form.pass, this.form.opinion)
